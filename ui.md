...
1. 展示当前电脑的内网 IP 和公网 IP。
2. 展示连接状态：是否已连接服务器、是否已连接主电脑。
3. 支持把当前电脑设置为主电脑，由当前电脑采集键盘、鼠标事件并同步给其他电脑。
...
- `role`: `master` 表示当前电脑是主电脑，`client` 表示当前电脑是被控电脑。
- `master_device_id`: 当前电脑不是主电脑时，记录期望连接的主电脑；当前电脑是主电脑时为 `null`。
- `layout`: 主电脑模式下的四方向绑定。
...
- 当前电脑是主电脑：状态显示“当前电脑就是主电脑”。
- 当前电脑不是主电脑：显示是否有主电脑正在连接或最近连接过本机。
