...
!define APP_NAME "KMSync Daemon"
!define APP_PUBLISHER "KMSync"
!define APP_EXE "kmsync-daemon.exe"
!define SERVICE_NAME "KMSyncCoreService"
!define SERVICE_DISPLAY_NAME "KMSync Core Service"

Name "${APP_NAME}"
OutFile "..\..\dist\windows\kmsync-daemon-${APP_VERSION}-windows-x64-setup.exe"
...
Section "Install"
  ...
  CreateShortCut "$SMPROGRAMS\KMSync\KMSync daemon info.lnk" "$INSTDIR\${APP_EXE}" "info"
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "KMSyncDaemon" '"$INSTDIR\${APP_EXE}" core-service "$INSTDIR\configs\daemon.example.json"'

  WriteUninstaller "$INSTDIR\Uninstall.exe"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KMSyncDaemon" "DisplayName" "${APP_NAME}"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KMSyncDaemon" "Publisher" "${APP_PUBLISHER}"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KMSyncDaemon" "DisplayVersion" "${APP_VERSION}"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KMSyncDaemon" "UninstallString" "$INSTDIR\Uninstall.exe"
SectionEnd
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\KMSync\KMSync daemon info.lnk"
  Delete "$INSTDIR\Uninstall.exe"
  RMDir "$INSTDIR\configs"
  RMDir "$INSTDIR\docs"
  RMDir "$SMPROGRAMS\KMSync"
  RMDir "$INSTDIR"
  DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Run" "KMSyncDaemon"
  DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\KMSyncDaemon"
SectionEnd
